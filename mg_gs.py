...
def main():
    ...
    def prolong(phih):
        ...
        phi = np.zeros(n+1)
        for i in range(1,n):
            if (i%2==0) : phi[i] = phih[int(i/2)]
            if (i%2==1) : phi[i] = (phih[int(i/2)]+phih[int(i/2+1)])/2
        return phi
    ...
    rho   = np.zeros(N+1)
    rho[int(N/2)] = 1./DX
    x     = np.linspace(0, int(LENGTH), N+1)
    ans   = (abs(0.5-x)-0.5)/2              # answer

    # Plain GS
    print("PLAIN GS", file=stderr)
    phi        = np.zeros_like(phi)
    start_time = time.time()
    for it in range(100000):
        phi = iterate(phi ,rho)
        err = relative_error(ans, phi)
        print("iter, err = %d, %g" % (it, err), '\r', end=' ', file=stderr)
        if err < 1.e-10 : break
    print("", file=stderr)
    print("elapsed_time = ",time.time()-start_time, file=stderr)
    #print "phi=", phi
